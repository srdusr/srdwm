...
/// The one line srdwm adds to the user's own `gtk.css`. First in the file,
/// because CSS `@import` is only valid before other rules - which also
...
/// `background-image` from a compiled gresource rather than as a child
...
///
/// Every style in `GTK_BUTTON_STYLES` is written out. The configured one is
/// live; the others are commented out below it, so the file shows what the
/// alternatives are and what each of them actually does.
///
/// Only `button_style` matters here. Which *side* the buttons sit on is not
/// a CSS question - that is the desktop's button-layout preference, which
/// `publish_gtk_button_layout` handles.
/// The look of srdwm's own titlebar, for a client that draws its own.
///
/// A client-side-decorated application draws its header itself, and no
/// compositor protocol reaches that - GTK never negotiates decoration at
/// all. So an app like Nemo keeps its own header bar whatever srdwm is
/// configured to do, and the desktop ends up with two different-looking
/// titlebars: srdwm's on every ordinary window, the app's own on that one.
///
/// This is the part that can be closed. The stylesheet gives a GTK header
/// bar the same background, text colour, height and title alignment srdwm
/// draws for itself, so the two read as one style even though two different
/// pieces of software draw them.
///
/// Deliberately not set: anything that would change what fits in the bar. A
/// GNOME app's header bar is its whole toolbar - search, menus, view
/// switches - so `min-height` can only raise the floor to srdwm's own
/// height, never cap it, and no padding or spacing is touched.
pub(crate) struct TitlebarLook {
    pub(crate) traffic_lights: bool,
...
fn gtk_titlebar_css(look: &TitlebarLook) -> String {
    let active = if look.traffic_lights { "traffic_lights" } else { "traditional" };
    let mut out = String::from(
        "/* Generated by srdwm from theme.decorations.title_bar.button_style.\n\
        \x20  Rewritten on every start and every config reload, so an edit here does\n\
        \x20  not survive. To change the style, set button_style in your srdwm\n\
        \x20  config. To change something no style covers, write the rule in gtk.css\n\
        \x20  instead: it is yours, srdwm only ever adds the one @import line to it,\n\
        \x20  and rules there come after this import and therefore win.\n\n\
        \x20  Each style below is a complete set of rules. The configured one is\n\
        \x20  live and the rest are commented out, so you can read what any of them\n\
        \x20  would do, or paste one into gtk.css and change it. */\n\n",
    );
    out.push_str(&format!(
        "/* srdwm's own titlebar look, for header bars this compositor cannot\n   draw itself. Only colour, height and title alignment: padding and\n   spacing are left alone, because a header bar is also some apps' whole\n   toolbar. */\n\nheaderbar,\nheaderbar.titlebar {{\n  background-image: none;\n  background-color: {bg};\n  color: {fg};\n  min-height: {h}px;\n}}\n\nheaderbar .title,\nheaderbar .subtitle {{\n  color: {fg};\n}}\n\n",
        bg = rgb_css(look.background),
        fg = rgb_css(look.foreground),
        h = look.height,
    ));
    // GTK centres a header bar's title by default, so only the off case
    // needs saying - and it is said by pinning the title to the start,
    // which is what srdwm's own left-aligned titlebar looks like.
    ...
    for (name, body) in GTK_BUTTON_STYLES {
        if name == active {
            ...
            // The bodies hold no comments of their own, so wrapping one in a
            // block comment cannot terminate early.
            out.push_str(&format!("/* button_style = \"{name}\"\n\n{body}*/\n\n"));
        }
    }
    out
}

/// Writes srdwm's own GTK stylesheet and makes sure the user's `gtk.css`
/// imports it, for both GTK 3 and GTK 4.
///
/// A GTK app draws its own window buttons from its GTK theme, and no
/// compositor protocol reaches those (GTK never negotiates decoration at
/// all). A user stylesheet does, because it loads after the theme - so
/// this is the only channel by which `button_style` can govern a
/// self-decorating app, and without it "set the style once" stops at
/// srdwm's own titlebars.
///
/// srdwm writes only its own file. The user's `gtk.css` gets one `@import`
/// line added if it is missing, and is otherwise never touched: it is
/// theirs, it routinely holds hand-written work, and silently rewriting it
/// would destroy customisation this code cannot understand. Their rules
/// come after the import and therefore still win.
///
...
fn publish_gtk_stylesheet(wm: &Rc<RefCell<WindowManager>>, nested: bool) {
    ...
    let look = {
        let wm = wm.borrow();
        TitlebarLook {
            traffic_lights: wm.theme.traffic_light_buttons,
            background: wm.theme.titlebar_bg,
            foreground: wm.theme.titlebar_fg_focused,
            height: srdwm_core::TITLEBAR_HEIGHT,
            title_centered: wm.theme.title_centered,
        }
    };
    let css = gtk_titlebar_css(&look);
    for version in ["gtk-3.0", "gtk-4.0"] {
        let dir = std::path::Path::new(&home).join(".config").join(version);
        if !dir.is_dir() {
            continue;
        }
        if let Err(e) = std::fs::write(dir.join(GTK_STYLE_FILE), &css) {
            log::debug!("couldn't write {version}/{GTK_STYLE_FILE} ({e}); GTK keeps its own button style");
            continue;
        }
        let user_css = dir.join("gtk.css");
        let existing = std::fs::read_to_string(&user_css).unwrap_or_default();
        if existing.contains(GTK_STYLE_IMPORT) {
            continue;
        }
        // Prepended, not appended: `@import` is only valid before any other
        // rule, and putting it first is also what leaves the user's own
        // rules able to override these.
        let merged = format!("{GTK_STYLE_IMPORT}\n\n{existing}");
        if let Err(e) = std::fs::write(&user_css, merged) {
            log::debug!("couldn't add the srdwm import to {version}/gtk.css ({e})");
        } else {
            log::info!("added {GTK_STYLE_IMPORT} to {version}/gtk.css");
        }
    }
}
...
fn apply_general_settings(engine: &Engine, wm: &Rc<RefCell<WindowManager>>) {
    ...
    // Maximize runs under a bottom dock by default - see
    // `WindowManager::maximize_covers_dock`.
    let maximize_covers_dock = engine.get_bool("general.maximize_covers_dock", true);
    ...
    wm.maximize_covers_dock = maximize_covers_dock;
...
mod tests {
    ...
    fn look(traffic_lights: bool) -> TitlebarLook {
        TitlebarLook { traffic_lights, background: (46, 52, 64), foreground: (236, 239, 244), height: 32, title_centered: true }
    }

    /// Strips every `/* ... */` block, so what is left is the CSS a GTK
    /// parser would actually apply.
    ...
    #[test]
    fn every_style_is_written_and_only_the_configured_one_is_live() {
        for (traffic_lights, active, inactive) in
            [(true, "9999px", "-gtk-icontheme"), (false, "-gtk-icontheme", "9999px")]
        {
            let css = gtk_titlebar_css(&look(traffic_lights));
            // Both styles are in the file, so it reads as the menu of what
            // button_style can be...
            assert!(css.contains("9999px"), "traffic_lights rules missing");
            assert!(css.contains("-gtk-icontheme"), "traditional rules missing");
            // ...but only the configured one is outside a comment.
            let live = uncommented(&css);
            assert!(live.contains(active), "configured style is commented out");
            assert!(!live.contains(inactive), "the other style is live too");
        }
    }

    #[test]
    fn the_reset_applies_whichever_style_is_configured() {
    ...
    /// The whole point of the titlebar block: a client that draws its own
    /// header must be able to end up looking like srdwm's.
    #[test]
    fn the_stylesheet_carries_srdwms_own_titlebar_colours_and_height() {
        let mut l = look(false);
        l.background = (10, 20, 30);
        l.foreground = (200, 210, 220);
        l.height = 44;
        let live = uncommented(&gtk_titlebar_css(&l));
        assert!(live.contains("background-color: rgb(10, 20, 30)"));
        assert!(live.contains("color: rgb(200, 210, 220)"));
        assert!(live.contains("min-height: 44px"));
    }

    /// A header bar is some applications' entire toolbar, so the stylesheet
    /// must never cap its height or touch what fits inside it.
    #[test]
    fn the_titlebar_block_never_caps_a_header_bars_size() {
        for centered in [true, false] {
            let mut l = look(false);
            l.title_centered = centered;
            let live = uncommented(&gtk_titlebar_css(&l));
            // Only the header bar's own rule: the button reset further down
            // sets `padding: 0` on a titlebutton, which is correct there.
            let start = live.find("headerbar,\nheaderbar.titlebar {").expect("no header bar rule");
            let block = &live[start..start + live[start..].find('}').expect("unterminated rule")];
            // Declarations, not substrings: "min-height" legitimately
            // contains "height".
            for property in ["height", "max-height", "padding", "font-size", "margin"] {
                let set = block.lines().any(|line| line.trim_start().starts_with(&format!("{property}:")));
                assert!(!set, "the header bar rule sets {property}");
            }
        }
    }
